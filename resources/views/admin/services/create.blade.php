<x-admin-layout>
    <x-slot name="header">Novo Serviço</x-slot>

    <div class="animate-fade-in max-w-4xl" x-data="{
        duration: '30',
        schedules: {
            0: { enabled: false, start_time: '08:00', end_time: '18:00', slots: [], selectedSlots: [], selectAll: true },
            1: { enabled: true, start_time: '08:00', end_time: '18:00', slots: [], selectedSlots: [], selectAll: true },
            2: { enabled: true, start_time: '08:00', end_time: '18:00', slots: [], selectedSlots: [], selectAll: true },
            3: { enabled: true, start_time: '08:00', end_time: '18:00', slots: [], selectedSlots: [], selectAll: true },
            4: { enabled: true, start_time: '08:00', end_time: '18:00', slots: [], selectedSlots: [], selectAll: true },
            5: { enabled: true, start_time: '08:00', end_time: '18:00', slots: [], selectedSlots: [], selectAll: true },
            6: { enabled: false, start_time: '08:00', end_time: '12:00', slots: [], selectedSlots: [], selectAll: true }
        },
        init() {
            this.updateAllSlots();
        },
        ...
        updateSlots(dayNum) {
            ...
            if (schedule && schedule.enabled && schedule.start_time && schedule.end_time) {
                ...
                // If selectAll is true, select all slots
                if (schedule.selectAll) {
                    schedule.selectedSlots = [...schedule.slots];
                } else {
                    // Keep only selected slots that are still valid
                    schedule.selectedSlots = schedule.selectedSlots.filter(slot => schedule.slots.includes(slot));
                }
            } else if (schedule) {
                schedule.slots = [];
        ...
        toggleSlot(dayNum, slot) {
            ...
            if (index > -1) {
                ...
                // Check if all slots are now selected
        ...
        prepareSubmit(e) {
            ...
            // Check if at least one slot is selected for each enabled day
            for (let i = 0; i <= 6; i++) {
        ...
        <form method="POST" action="{{ route('admin.services.store') }}" @submit="prepareSubmit">
            @csrf
            ...
            <div class="rounded-2xl bg-white dark:bg-dark-800 shadow-soft overflow-hidden mb-6">
                <div class="border-b border-dark-100 dark:border-dark-700 px-6 py-4">
                    <h2 class="text-lg font-semibold text-dark-900 dark:text-white">Informações do Serviço</h2>
                    <p class="text-sm text-dark-500 dark:text-dark-400">Preencha os dados básicos do serviço.</p>
                </div>
